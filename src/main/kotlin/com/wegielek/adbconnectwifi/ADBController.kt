...
class ADBController {
    ...
    private val deviceService = service<DeviceIpService>()
    ...
    fun connectDevice() {
        try {
            ...
            val connectedUSBDevices = AdbUtils.getConnectedDevices()
                ...
                .filter {
                    !it.substringAfter("(").substringBefore(")").all { letter ->
                        letter.isDigit() || letter == '.' || letter == ':'
                    }
                }
            ...
            deviceService.saveDevice(deviceId + " [saved]", ip)
            Messages.showInfoMessage(
    ...
    fun disconnectDevice() {
        try {
            ...
            result?.let {
                Messages.showInfoMessage(
                    "Disconnected from $deviceId at $deviceIp\nResult: $it",
                    "ADB Wi-Fi Success"
                )
            }
        } catch (ex: Exception) {
    ...
    fun resetDevices() {
        ...
        Messages.showInfoMessage("Removed all saved devices.", "Reset Devices")
    }
}
